		...
		if (probe && diff>=sleep)
		{
			...
			if (estimated>bitrate)
			{
				...
				for (auto &group : outgoing)
				{
					...
					{
						UltraDebug("-DTLSICETransport::Run() | Sending probing packets [estimated:%u,bitrate:%u,probing:%u,num:%d,sleep:%d]\n", estimated, bitrate,probingBitrate, num, sleep);
						//Set all the probes
