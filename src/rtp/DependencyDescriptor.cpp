...
std::optional<TemplateDependencyStructure> TemplateDependencyStructure::Parse(BitReader& reader)
{
	...
	tds->templateIdOffset	= reader.Get(6);	
	tds->dtsCount		= reader.Get(5) + 1;	
	...
	while (nextLayerIdc!=NoMoreLayers)
	{
		...
		//Get next layer id
		nextLayerIdc = (NextLayerIdc)reader.Get(2); 
	...
	for (auto& fdt : tds->frameDependencyTemplates)
	{
		...
		while(fdt.decodeTargetIndications.size()< tds->dtsCount)
		{
			...
			fdt.decodeTargetIndications.push_back((DecodeTargetIndication)reader.Get(2)); 
		}
	}
	...
	for (auto& fdp : tds->frameDependencyTemplates)
	{
		...
		while (reader.Get(1))
		{
			...
			fdp.frameDiffs.push_back(reader.Get(4) + 1); 
		}
	}
	...
	if (tds->chainsCount) 
	{
		...
		while (tds->decodeTargetProtectedByChain.size() < tds->dtsCount)
		{
			...
			tds->decodeTargetProtectedByChain.push_back(reader.GetNonSymmetric(tds->chainsCount)); 
		}
		...
		for (auto& fdt : tds->frameDependencyTemplates)
		{
			...
			while (fdt.frameDiffsChains.size()<tds->chainsCount)
			{
				...
				fdt.frameDiffsChains.push_back(reader.Get(4)); 
			}
		}
	}
	...
	if (reader.Get(1)) 
	{
		...
		{
			//Read resolution
			uint32_t maxRenderWidthMinusOne  = reader.Get(16); 
			uint32_t maxRenderHeightMinusOne = reader.Get(16); 
...
bool TemplateDependencyStructure::Serialize(BitWriter& writter) const
{
	try
	{
		...
		//template_chains()
		writter.WriteNonSymmetric(dtsCount+1, chainsCount );
  
		//Read all decode targets
		for (auto& chain : decodeTargetProtectedByChain)
			//template_chains()
			writter.WriteNonSymmetric(chainsCount, chain);
